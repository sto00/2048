...
//TODO 확장성 ex 블록의 개수가 늘어나거나 줄어들면 못만든다.
//TODO 다양한 버그
//TODO 필요하지 않은 변수
...
public class sto2048 : MonoBehaviour
{
    ...
    */
    ...
    enum DirectionType { Up, Down, Right, Left }


    bool IsBlockOnLimit(DirectionType directionType, NumberBlock block)
    {
        switch (directionType)
        {
            case DirectionType.Up:
                return block.NumberPositionY <= 0;
            case DirectionType.Down:
                return block.NumberPositionY >= 3;
            case DirectionType.Right:
                return block.NumberPositionX >= 3;
            case DirectionType.Left:
                return block.NumberPositionX <= 0;
    ...
    void TransferBlock(DirectionType directionType, int index)
    {
        switch (directionType)
        {
            case DirectionType.Up:
                numberBlocks[index].NumberPositionY -= 1;
                break;
            case DirectionType.Down:
                numberBlocks[index].NumberPositionY += 1;
                break;
            case DirectionType.Right:
                numberBlocks[index].NumberPositionX += 1;
                break;
            case DirectionType.Left:
                numberBlocks[index].NumberPositionX -= 1;
                break;
    ...
    void MoveBlocks(DirectionType directionType)
    {
        for (int i = 0; i < numberBlocks.Count; i++)
        {
            bool isLimit = IsBlockOnLimit(directionType, numberBlocks[i]);
            ...
            bool isBlock = IsThereBlock(numberBlocks[i], directionType); // 상하좌우 다름 
            if (isBlock == false)
            {
                // 블록이 아직 이동할 수 있는 경우 
                while ((isBlock == false) && IsBlockOnLimit(directionType, numberBlocks[i]) == false)
                {
                    TransferBlock(directionType, i);
                    xToMove = numberBlocks[i].NumberPositionX;
                    yToMove = numberBlocks[i].NumberPositionY;
                    isBlock = IsThereBlock(numberBlocks[i], directionType);
                }
            }
            if (isBlock == true)
            {
                ...
                NumberBlock targetBlock = GetBlockOrNull(numberBlocks[i],directionType); 
    ...
    void MoveLeft()
    {
        // 모든 블록을 순회하면서, 이동을 처리한다. 
        //선언 , 조건(끝나는), 반복되는 내용
        for (int i = 0; i < numberBlocks.Count; i++)
        {

            if (numberBlocks[i].NumberPositionX <= 0) // TODO 상하좌우 다름 
            {
                continue;
            }
            int xToMove = numberBlocks[i].NumberPositionX;
            int yToMove = numberBlocks[i].NumberPositionY;
            bool isBlock = IsThereBlock(xToMove - 1, yToMove); // 상하좌우 다름 
            if (isBlock == false)
            {
                // 블록이 아직 이동할 수 있는 경우 
                while ((isBlock == false) && numberBlocks[i].NumberPositionX > 0) //TODO  상하좌우 다름 
                {
                    numberBlocks[i].NumberPositionX = numberBlocks[i].NumberPositionX - 1; //TODO 
                    xToMove = numberBlocks[i].NumberPositionX;
                    yToMove = numberBlocks[i].NumberPositionY;
                    isBlock = IsThereBlock(xToMove - 1, yToMove); //TODO 
                }
            }
            if (isBlock == true)
            {
                // 블록이 합성되는 경우 
                // 이동해야 하는 곳에 있는 블록이 같은 사이즈인 경우
                NumberBlock targetBlock = GetBlockOrNull(xToMove - 1, yToMove); //TODO 
                int size = numberBlocks[i].NumberSize;
                if (targetBlock.NumberSize == size)
                {
                    numberBlocks[i].NumberPositionX = numberBlocks[i].NumberPositionX - 1; //TODO 
    ...
    bool IsThereBlock(NumberBlock block, DirectionType directionType)
    {
        int xPos = block.NumberPositionX;
        int yPos = block.NumberPositionY;
    ...
    NumberBlock GetBlockOrNull(NumberBlock block , DirectionType directionType)
    {
        int xPos = block.NumberPositionX;
        int yPos = block.NumberPositionY;
        ...
        return GetBlockOrNull(xPos, yPos);
    }
    ...
    void Update()
    {
        ...
        {
            MoveBlocks(DirectionType.Left);
        }
        if (Input.GetKeyUp(KeyCode.RightArrow) == true)
        {
            //MoveRight();
            MoveBlocks(DirectionType.Right);
        }
        if (Input.GetKeyUp(KeyCode.UpArrow) == true)
        {
            //MoveUp();
            MoveBlocks(DirectionType.Up);
        }
        if (Input.GetKeyUp(KeyCode.DownArrow) == true)
        {
            //MoveDown();
            MoveBlocks(DirectionType.Down);
        }
    }
